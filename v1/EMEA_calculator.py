...
def run_3():
          ...
          client_name = colbb.text_input("", key="client_name_EMEA")
          ...
          managed_spend = st.number_input("Total Managed Spend (USD)",min_value=0,key="managed_spend_EMEA",placeholder="Type a number...")
          ...
          for i, var_name in enumerate(keys):
            ...
            selected_option = st.selectbox(f"{i+1}.{var_name}", list(options.keys()), key=f"{var_name}_EMEA")
          ...
          if st.session_state.get('reset_triggered',False):
              for activity in AR_activities + Reporting_activities + Client_Reporting_Activities + Special_client_activity:
                    st.session_state[f"{activity}_EMEA"] = 'Monthly'
                    st.session_state['reset_triggered'] = False
                
          def reset_frequencies():
                 for activity in AR_activities + Reporting_activities + Client_Reporting_Activities + Special_client_activity:
                                st.session_state[f"{activity}_EMEA"] = 'None'
          ...
          for activity in AR_activities: 
              selected_frequency = st.selectbox(f"{activity}", list(frequency_options.keys()), key=f"{activity}_EMEA")
          ...
          selected_activities = st.radio("21.Client Reporting - select one of the below :",Special_client_activity,key="activity_selection_EMEA")
          # frequency_options4 = {'Monthly': 1,'None': 0, 'Weekly': 4, 'Bi-Weekly': 2, 'Quarterly': 0.333333333333333, 'Yearly': 0.083}
          selected_frequency1 = st.selectbox(f"{selected_activities}", list(frequency_options.keys()), key=f"{selected_activities}_EMEA")
          ...
          if st.button('Reset Frequencies to Monthly', key="reset_EMEA"):
          ...
          selected_delivery_model=st.selectbox("Select Delivery Model",delivery_model,key="delivery_EMEA")
          ...
          with col5:
              pass
          with col3 :
              center_button = st.button('Calculate FTE Requirement',key="Calc_EMEA")
          if center_button:
              st.session_state['data_entries']=pd.DataFrame()
              entries = []
          ...
          # Check if the session state DataFrame is empty
              ...
              st.subheader("EMEA Activities Summary")
