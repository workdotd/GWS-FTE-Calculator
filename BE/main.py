...
var_data = pd.read_excel("Inputs.xlsx", 'Sheet1')
...
var_data = pd.read_excel("Inputs.xlsx", 'Sheet1')
activities = var_data['All_activities'].dropna()
...
LCA = var_data['LCA'].dropna()
MCA = var_data['MCA'].dropna()
HCA = var_data['HCA'].dropna()
...
def fmanagement(radf: pd.DataFrame):
    ...
    def determine_role(row):
        ...
        if row['RA Region'] == "APAC":
            ...
            if row['Contract Value'] == "None" and row['Performance Risk'] == "None" and row['Growth Opportunity'] == "None":
                return "None"
            elif (row["Contract Value"] == cv2 and row["Performance Risk"] == pr2 and row["Growth Opportunity"] == go2) or (row["Contract Value"] == cv2 and row["Performance Risk"] == pr2 and row["Growth Opportunity"] != go2) or (row["Contract Value"] == cv2 and row["Performance Risk"] != pr2 and row["Growth Opportunity"] == go2) or (row["Contract Value"] != cv2 and row["Performance Risk"] == pr2 and row["Growth Opportunity"] == go2):
                return "Sr Finance Manager"
    ...
    fmdf['RA FTE Count'] = np.where(radf['RA Calc Role'].ne("None"), 1, 0)
    fmdf['RA Location'] = "TBC"
    
    ftec_global = 0
    garr = None
    if radf["Global Role Req"].eq("No").any():
        if fmdf['GA Region_count'].gt(1).any() and "Above 50M Managed Spend" in fmdf["Global Managed Spend"].values:
            garr = "Global Finance Director"
            ftec_global = 1
            
        elif fmdf['GA Region_count'].gt(1).any() and "Below 50M Managed Spend" in fmdf["Global Managed Spend"].values:
            if "Finance Director" in fmdf['RA Calc Role'].values:
                garr = "Finance Director (Double Hat)"
                ftec_global = 1
            elif "Sr. Finance Manager" in fmdf['RA Calc Role'].values:
                garr = "Sr. Finance Manager (Double Hat)"
                ftec_global = 1
            else:
                garr = "No Role Recomended"
                ftec_global = 0
    ...
    fmsumm = pd.DataFrame({"Role": [garr,"-----",fmdf['RA Calc Role'][0],fmdf['RA Calc Role'][1],fmdf['RA Calc Role'][2],""],"location":["-----","-----", "TBC", "TBC", "TBC",""],"FTE":[ftec_global,"-----",ftec_ams,ftec_emea,ftec_apac,total_ftec]} ,index=["Global Role","Regional Roles","AMERICAS","EMEA","APAC","Total Finance Management"])
...
def fmsumdnld(radf: pd.DataFrame):
    ...
    fmsum = fmanagement(radf)
    try:
        fmsum_path = "fm_summary.csv"
        fmsum.to_csv(fmsum_path)
        return fmsum_path
...
def calculate_fte_requirement(df: pd.DataFrame):
    ...
    for index, row in inputs.iterrows():
        ...
        if f'{activity}_freq' in df.columns:
            ...
            if len(frequency_values) > 0:
                ...
                df.loc[df['Country'] == country, f'{activity}'] = round(time_spent * multiplier,2)


    df['Total No of Sites'] = df['Number of Sites'].sum()
    df['Average Spend per Site'] = round(df['Total Managed Spend']/df['Total No of Sites'],2)
    df['Spend per Country'] = round(df['Average Spend per Site'] * df['Number of Sites'],2)
    ...
    df['Adjusted No of Sites/Country'] = round(df['Number of Sites']*df['Average Site Multiplier value'],2)
    df['Adjusted No of Sites'] = df['Adjusted No of Sites/Country'].sum()

    df['1.Billing_adj'] = np.where(df['Adjusted No of Sites/Country'] <= 1,df['1.Billing'] * 2 ,round(df['1.Billing']*df['Adjusted No of Sites/Country'],2))
    ...
    fte_count_xl_ss = mt.ceil(df['after_add_overlay'].sum()/150)
    fte_count_xl_os = mt.ceil((1.3)*df['after_add_overlay'].sum()/150)
    fte_count_xl_m = mt.ceil(fte_count_xl_ss*lowpct) + mt.ceil(fte_count_xl_os*hmpct)

    fte_count_ml_ss = mt.ceil(df["ML Prediction"].sum()/150)
    fte_count_ml_os = mt.ceil((1.3)*df["ML Prediction"].sum()/150)
    fte_count_ml_m = mt.ceil(fte_count_ml_ss*lowpct) + mt.ceil(fte_count_ml_os*hmpct)

    # Shared Services
    shared_services_mask = df['Delivery Model'] == "Shared Services"
    df.loc[shared_services_mask, 'SFAE'] = mt.ceil(fte_count_xl_ss * hmpct)
    df.loc[shared_services_mask, 'SFAML'] = mt.ceil(fte_count_ml_ss * hmpct)
    ...
    df.loc[on_site_mask, 'SFAE'] = mt.ceil(fte_count_xl_os * hmpct)
    df.loc[on_site_mask, 'SFAML'] = mt.ceil(fte_count_ml_os * hmpct)
    ...
    df.loc[mixed_mask, 'SFAE'] = mt.ceil(fte_count_xl_os * hmpct)
    df.loc[mixed_mask, 'SFAML'] = mt.ceil(fte_count_ml_os * hmpct)
    ...
    dfdb = df.iloc[:,:71]
    dfdb.drop(dfdb.columns[[64,69]],axis=1,inplace = True)   
    dfdb['Final_Est_Hours'] = round(df.iloc[:,85].sum(),2)
    dfdb['Total_Est_FTE'] = tftec
    dfdb['FA'] = FAE
    dfdb['SFA'] = SFAE
    dfdb['FM'] = tle
    dfdb['Final_Est_Hours_ML'] = round(df.iloc[:,101].sum(),2)
    ...
    dbdf = dfdb.rename(columns = {'Average Spend per Site':'Avg_Spend_Site',
                                ...
                                'Adjusted No of Sites/Country':'Adj_Nof_Sites',
    ...
    for index, row in inputs.iterrows():
        ...
        if f'{activity}_freq' in df.columns:
            ...
            if len(frequency_values) > 0:
                ...
                # Update the df with the calculated value
                df.loc[df['Country'] == country, f'{activity}'] = round(time_spent * multiplier)
...
def download_summary(df: pd.DataFrame):
    dfam,dft = calculate_fte_requirement(df)
    actable = activity_summary(df)
...
async def calculate_fte(request:Request):
     try:
        ...
        fte_summ,dft = calculate_fte_requirement(df_1)
